...
export default function EmotionSelect({
  ...
  return (
    <Motion.div
      key={'emotion-select'}
      className={`${title ? `h-full responsive-p-t` : ``} pb-20 flex flex-col items-center justify-center gap-10`}>
      {title && <p className={`text-[32px] max-md:text-[28px] max-sm:text-[24px] font-semibold select-none transition-all-300`}>{title}</p>}
